import {
  CircularProgress, IconButton, styled, Theme, Typography,
} from '@mui/material';
...
export default function ({
  showFiles, checkFile, onClick, loading,
}: IMagicButton) {
  return (
    <MButtonContainer showFiles={showFiles} checkFile={checkFile} onClick={onClick}>
      <MSpinner
        loading={loading}
        color="secondary"
        size="small"
      ...
      {/* <MTitle loading={loading}
        variant='h5'>{process.env.REACT_APP_NAME?.toUpperCase()}</MTitle> */}
    </MButtonContainer>
  );
}
