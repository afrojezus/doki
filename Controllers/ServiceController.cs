    ...
    public class ServiceController : Controller
    {
        ...
        public async Task<ActionResult<int>> DeleteAuthorAndFiles([FromBody] RemovalForm r)
        {
            try
            {
                ...
                {
                    await _service.Delete(file.Id, _hostingEnvironment.ContentRootPath);
                }
        ...
        public async Task<ActionResult<IEnumerable<File>>> UploadFile([FromForm] UploadForm upload)
        {
            ...
            foreach (var file in upload.File)
            {
                ...
                var targetFileName =
                    $"{_hostingEnvironment.ContentRootPath}/app/build/files/{sanitizedFileName}";
        ...
        public async Task<ActionResult<IEnumerable<File>>> DeleteFile(int id, [FromForm] DeleteForm delete)
        {
            if (delete.Id != null)
            {
                var result = await _service.Delete(id, _hostingEnvironment.ContentRootPath);
                if (result > 0)
                    return Ok(await Get());
                else
                    return NotFound();
        ...
        [Route("update/{id}")]
        [HttpPost]
        public async Task<ActionResult<IEnumerable<File>>> UpdateFile(int id, [FromForm] UpdateForm update)
        {
            if (update.Id != null)
            {
                var result = await _service.FindOne(id);
